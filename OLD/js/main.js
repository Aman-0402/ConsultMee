...
function fetchConsultants() {
  fetch('./assets/scripts/fetch_consultants.php')
    ...
    .then(data => {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        document.getElementById('appointmentForm').addEventListener('submit', async function (e) {
          ...
          fetch('./assets/scripts/book_appointment.php', {
            method: 'POST',
            headers: {
...
document.addEventListener("click", function (e) {
  ...
  if (e.target.classList.contains("view-profile-btn")) {
    ...
    window.open(`consultant_profile.php?username=${encodeURIComponent(username)}`, "_blank");
  }
...
function fetchCategories() {
  fetch('./assets/scripts/fetch_categories.php')
    .then(response => response.json())
    .then(data => {
      if (data.error) {
...
function fetchUser() {
  return fetch('./assets/scripts/get_user_details.php')
    .then(res => res.json())
    .then(data => {
      if (data.error) {
...
function fetchConsultantsByCategory(categoryName) {
  fetch(`./assets/scripts/fetch_category_consultant.php?category=${encodeURIComponent(categoryName)}`)
    .then(res => res.json())
    .then(data => {
      const container = document.getElementById("consultants");
...
function fetchAppointments() {
  fetch('./assets/scripts/fetch_user_appointments.php')
    .then(res => res.json())
    .then(data => {
      ...
      container.querySelectorAll('.cancel-btn').forEach(button => {
        button.addEventListener('click', () => {
          ...
          if (confirm("Are you sure you want to cancel this appointment?")) {
            fetch('./assets/scripts/update_appointment_status.php', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json'
              },
...
function fetchHistory() {
  fetch("./assets/scripts/fetch_user_history.php")
    .then(res => res.json())
    .then(data => {
...
function submitRating(appointmentId, consultantUsername, userName) {
  ...
  fetch("./assets/scripts/update_user_rating.php", {
    method: "POST",
    headers: {
...
function fetchUserDetails() {
  fetch('./assets/scripts/fetch_user_profiles.php')
    .then(res => res.json())
    .then(data => {
      if (data.error) {
        alert(data.error);
...
function saveUserProfile() {
  ...
  xhr.open("POST", "./assets/scripts/update_user_profile.php", true);
...
const PROJECT_FETCH   = "./assets/scripts/fetch_projects.php";
const PROJECT_CREATE  = "./assets/scripts/create_project.php";
const PROJECT_APPS    = "./assets/scripts/fetch_project_applications.php";
