...
function fetchAppointments() {
  fetch('./assets/scripts/consultant_appointments.php', { credentials: 'same-origin' })
    .then(response => {
...
function saveAppointment() {
  ...
  fetch('./assets/scripts/update_appointments.php', {
    method: 'POST',
...
function fetchConfirmedAppointments() {
  fetch('./assets/scripts/fetch_confirmed_appointments.php', { credentials: 'same-origin' })
    .then(response => {
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
function fetchHistory() {
  fetch('./assets/scripts/fetch_completed_appointments.php', { credentials: 'same-origin' })
    .then(response => {
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
function fetchConsultantProfile() {
  ...
  fetch('./assets/scripts/fetch_consultant_profile.php', { credentials: 'same-origin' })
    .then(response => {
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
function saveProfile() {
  ...
  xhr.open("POST", "./assets/scripts/update_consultant_profile.php", true);
...
function showConsultantProjects(searchText = '', page = 1) {
  ...
  fetch('./assets/scripts/fetch_projects_consultant.php?' + params.toString(), {
    credentials: 'same-origin'
  })
    .then(res => {
...
function submitProjectApplication() {
  ...
  fetch('./assets/scripts/apply_project.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
    credentials: 'same-origin'
