<main class="flex-1 flex items-center justify-center px-4 py-16">
  <div class="w-full max-w-md">
    <div class="text-center mb-8">
      ...
      <p class="text-slate-500 text-sm">Enter your consultant email to receive a reset OTP</p>
    </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
      ...
      <div class="border-t border-slate-100 mt-6 pt-5 text-center">
        <p class="text-slate-500 text-sm">Remembered? <a href="/consultant/login" class="text-cobalt hover:underline font-semibold">Sign in</a></p>
      </div>
    </div>
  </div>
</main>
...
<script>
  ...
  sendOtpForm.addEventListener('submit', async (e) => {
    ...
    const res = await fetch('/api/auth/consultant/send-otp', { method: 'POST', body: formData });
    let data;
  ...
  verifyOtpForm.addEventListener('submit', async (e) => {
    ...
    const res = await fetch('/api/auth/consultant/verify-otp', { method: 'POST', body: formData });
    let data;
    ...
    if (data.success) {
      ...
      setTimeout(() => { window.location.href = '/consultant/login'; }, 2000);
    } else {
